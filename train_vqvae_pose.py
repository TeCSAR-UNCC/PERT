...
from dalle_pytorch import VQVAE
...
for epoch in range(config.epochs):
    ...
    for i, heatmaps in enumerate(distr_dl):
        ...
        with torch.cuda.amp.autocast():
            loss, recons, codes = distr_vae(
                heatmaps, return_loss=True, return_recons=True
            )
        ...
        if i % 100 == 0:
            if distr_backend.is_root_worker():
                ...
                heatmaps, recons, codes = map(
                    lambda t: t.detach().cpu(), (heatmaps, recons, codes)
                )
