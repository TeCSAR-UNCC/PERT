...
class ImageEmbed(nn.Module):
    ...
    def __init__(
        ...
        enc_layers.append(nn.Conv2d(enc_chans[-1], vocab_size, 1))
    ...
    def _register_external_parameters(self):
        ...
        deepspeed.zero.register_external_parameter(self, self.proj.weight)
    ...
    def forward(
        ...
        sampled = torch.einsum(
            "b n h w, n d -> b d h w", soft_one_hot, self.proj.weight
        )
        N, ED, H, W = sampled.shape
        embed = sampled.permute(0, 2, 3, 1).reshape(N, H * W, ED)
        return embed
...
@register_model
def bept_base_patch16_224(pretrained=False, **kwargs):
...
@register_model
def bept_base_patch8_224(pretrained=False, **kwargs):
    if "pretrained_cfg" in kwargs:
        kwargs.pop("pretrained_cfg")
...
@register_model
def bept_base_patch16_384(pretrained=False, **kwargs):
    model = VisionTransformer(
        img_size=384,
        patch_size=16,
        embed_dim=768,
...
@register_model
def bept_large_patch16_224(pretrained=False, **kwargs):
    model = VisionTransformer(
        patch_size=16,
        embed_dim=1024,
        depth=24,
...
@register_model
def bept_large_patch8_224(pretrained=False, **kwargs):
    model = VisionTransformer(
        patch_size=8,
        embed_dim=1024,
        depth=24,
...
@register_model
def bept_large_patch16_384(pretrained=False, **kwargs):
    model = VisionTransformer(
        img_size=384,
        patch_size=16,
        embed_dim=1024,
...
@register_model
def bept_large_patch16_512(pretrained=False, **kwargs):
    model = VisionTransformer(
        img_size=512,
        patch_size=16,
        embed_dim=1024,
