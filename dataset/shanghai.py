...
class shanghai(Dataset):
    ...
    def __init__(self, data_dir, train=True, path_to_vid_dir=None, normalize_pose_segs=True, return_indices=False,
        ...
        self.transform_list = None# dataset_args.get('trans_list', None)
        ...
        self.global_data, self.segs_score_np = \
            gen_dataset(data_dir, self.eval, num_clips=num_clips, ret_keys=True,
    ...
    def __getitem__(self, index):
        ...
        if self.apply_transforms:
            ...
            data = data_numpy
            # data = self.transform_list[trans_index](data_numpy)
        else:
            sample_index = index
            data = np.array(self.segs_data_np[index])
            trans_index = 0  # No transformations

        # if self.normalize_pose_segs:
        #     self.args['vid_res'] = self.segs_meta[sample_index][-2:]
        #     data = normalize_pose(data[None, ...], **self.args).squeeze(axis=0)
        
        # if self.args['relative']:
        #     relative_movement = np.zeros_like(data)
        #     for i in range(1, data.shape[1]):
        #         relative_movement[:, i, :] = data[:, i,:] - data[:, 0, :]
        #     data = np.concatenate((data, relative_movement), axis=1)
        
        data = torch.Tensor(data[:, :, 0:2]).float()
        seg_len, num_kp, channels = data.shape
        data_flat = data.reshape(seg_len, -1)

        masked_amount = int(data_flat.shape[0] * 0.2)# self.mask_chance)
        mask = torch.cat((torch.ones(masked_amount, dtype=torch.bool), 
                        torch.zeros(data_flat.shape[0] - masked_amount, dtype=torch.bool)))
        mask = mask[torch.randperm(data_flat.shape[0])]
        mask = torch.cat((torch.tensor([0], dtype=torch.bool), mask))
        
        indices = torch.nonzero(mask).squeeze().tolist()
        cls_token = torch.ones_like(data_flat[0]).unsqueeze(0) * -1
        data_flat = torch.cat((cls_token, data_flat), dim=0)
        # ret_arr[0] = ret_arr[0].view(ret_arr[0].shape[0], -1).float()
        gt = data_flat[indices].clone()
        data_flat[mask] = 1.0
        ret_arr = [data_flat, gt, mask, 0, 0]
...
def get_dataset_and_loader(args, trans_list, only_test=False):
    loader_args = args.LOADER
    dataset_args = args.DATASET
    dataset, loader = dict(), dict()
    splits = ['train', 'test'] if not only_test else ['test']
    for split in splits:
        evaluate = split == 'test'
        # abnormal_train_path = None # args.pose_path_train_abnormal if split == 'train' else None
        ...
        dataset[split] = shanghai(args.data_dir,
                                    evaluate=evaluate,
                                    **dataset_args)
...
def gen_dataset(data_dir, test=True, num_clips=None, kp18_format=True, ret_keys=False, ret_global_data=True,
                **dataset_args):
    segs_data_np = []
    segs_score_np = []
    segs_meta = []
    ...
    datasets = dataset_args.get('dataset',['ShanghaiTech'])
    folder = 'test' if test else 'train'

    for dataset in datasets:
        person_json_root = os.path.join(data_dir, dataset, 'pose', folder)
        dir_list = os.listdir(person_json_root)
        if dataset=='c1' or dataset=='c2' or dataset=='c3' or dataset=='c4' or dataset=='combined' or dataset=='corridor' or dataset=='Avenue':
        ...
        for person_dict_fn in tqdm(json_list):
            if dataset == "UBnormal":
                ...
                if dataset == 'corridor' or dataset == 'Avenue':
    ...
    segs_data_np = segs_data_np.astype(np.float32)
    global_data_np = global_data_np.astype(np.float32)
