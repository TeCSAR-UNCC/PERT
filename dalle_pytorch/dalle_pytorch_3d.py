...
class Discrete3DVAE(nn.Module):
    ...
    def forward(
        ...
        sampled = einsum("b n j h w, n d -> b d j h w", one_hot, self.codebook.weight)
...
class CLIP(nn.Module):
    def __init__(
        ...
        *,
        ...
        assert (
            visual_image_size % visual_patch_size == 0
    ...
    def forward(self, text, image, text_mask=None, return_loss=False):
        ...
        image_patches = rearrange(
            image, "b c (h p1) (w p2) -> b (h w) (p1 p2 c)", p1=p, p2=p
        )
        image_emb = self.to_visual_embedding(image_patches)
        image_emb += self.visual_pos_emb(
            torch.arange(image_emb.shape[1], device=device)
        )
        ...
        text_latents, image_latents = map(
            lambda t: F.normalize(t, p=2, dim=-1), (text_latents, image_latents)
        )
        ...
        *,
        ...
        for cur_len in range(out.shape[1], total_len):
            ...
            logits = self.forward_with_cond_scale(
                text, image, cond_scale=cond_scale, cache=cache
            )
            logits = logits[:, -1, :]
