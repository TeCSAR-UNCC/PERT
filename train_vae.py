...
train_group.add_argument(
    "--starting_temp", type=float, default=10.0, help="starting temperature"
)
...
model_group.add_argument(
    "--num_layers", type=int, default=3, help="number of layers (should be 3 or above)"
)
...
ds = ImageFolder(
    ...
    T.Compose(
        [
            T.Lambda(
                lambda img: img.convert(IMAGE_MODE) if img.mode != IMAGE_MODE else img
            ),
            T.Resize(IMAGE_SIZE),
...
if distr_backend.is_root_worker():
    ...
    run = wandb.init(
        project="dalle_train_vae", job_type="train_model", config=model_config
    )
