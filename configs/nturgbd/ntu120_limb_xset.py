model = dict(
    ...
    cls_head=dict(type="I3DHead", in_channels=512, num_classes=120, dropout=0.5),
    test_cfg=dict(average_clips="prob"),
)

dataset_type = "PoseDataset"
ann_file = "/mnt/DATASETS_RAID_28TB/nturgbd/processed_pkl_2d/ntu120_hrnet.pkl"
...
data = dict(
    ...
    val=dict(
        type=dataset_type, ann_file=ann_file, split="xset_val", pipeline=val_pipeline
    ),
    test=dict(
        type=dataset_type, ann_file=ann_file, split="xset_val", pipeline=test_pipeline
    ),
)
# optimizer
optimizer = dict(
