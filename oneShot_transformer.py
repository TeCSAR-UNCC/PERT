...
def validate(model_engin, dl_validation, dl_exe, device, using_deepspeed, nprocs, temp=0.9):
    ...
    with torch.no_grad():  # Disable gradient calculation
        ...
        embeddings = {}
        for _, batch in enumerate(dl_exe):
            data, target = batch
            data, target = data.to(device), target.to(device)
            with torch.cuda.amp.autocast():
                outputs = model_engin(data, temp) # , return_embed=True

            embeddings[target[0].item()] = outputs
...
def main(args):
    ...
    # data training
    ds_train = eval("dataset." + config.DATASET.train_dataset)(config, loader_type='train')

    # data validation
    ds_eval = eval("dataset." + config.DATASET.test_dataset)(config, loader_type='val')

    # data validation
    ds_exe = eval("dataset." + config.DATASET.test_dataset)(config, loader_type='exemplar')
    ...
    if using_deepspeed:
        ...
        exe_sampler = DistributedSampler(
            ds_exe,
            num_replicas=nprocs,
            rank=args.local_rank,
            shuffle=False,
